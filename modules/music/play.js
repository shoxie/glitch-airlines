...
exports.exec = (Bastion, message, args) => {
  ...
  try {
    ...
    yt.getInfo(song, ['-q', '-i', '--skip-download', '--no-warnings', '--format=bestaudio[protocol^=http]'], (err, info) => {
      ...
      console.log(test)
      if (test.includes('Sia')) {
        const fs = require('fs');
        let lines = fs.readFileSync('./data/sia.txt').toString().split("\n");
        const randomm = lines[Math.floor(Math.random() * lines.length)];
        message.channel.send(message.author + ' Sia quoted : ' + randomm)
      };
      if (test.includes('BLACKPINK')) {
        message.channel.send('BLACKPINK in ' + message.author + ' area')
      };
      // textChannel.setTopic(`Playing ${info.title}`)
      textChannel.send({
        embed: {
